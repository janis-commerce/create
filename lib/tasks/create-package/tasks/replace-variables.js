...
const fixPackageJsonName = rootDir => {
	const actualName = path.resolve(rootDir, '_package.json');
	const expectedName = path.resolve(rootDir, 'package.json');
...
module.exports.task = async ctx => {
	...
	return fixPackageJsonName(packageRootDir);
};
